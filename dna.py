...
# read the dna sequence txt file
with open(argv[2]) as dnafile:
    dnareader = reader(dnafile)
    for row in dnareader:
        dnalist = row

dna = dnalist[0] # store it in a string
sequences = {} # dictionary to store the sequences we want to count

# extract the sequences from the csv database
